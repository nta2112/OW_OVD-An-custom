...
try:
    ...
    def _clean_info(self, msg, *args, **kwargs):
        msg_str = str(msg)
        if any(term in msg_str for term in ['YOLOWorldGTDetector(', 'MultiModalYOLOBackbone(', 'paramwise_options', 'Checkpoints will be saved to']):
            return
...
# model settings
model = dict(type='YOLOWorldGTDetector',
             mm_neck=True,
