...
def main():
    ...
    # Load class_names list from model config
    from mmengine.config import Config
    cfg = Config.fromfile(args.config)
    class_names_list = cfg.get('class_names', [])
    
    # Load category mapping from train.json to map class_id to 0-indexed position
    # In the 25 classes split, category IDs correspond to specific strings in class_names_list.
    # We dynamically map each category ID to its corresponding index in the class_names_list.
    ...
    if os.path.exists(train_json_path):
        try:
            ...
            for cat in coco_train.get('categories', []):
                cat_name = str(cat['name'])
                if cat_name in class_names_list:
                    cat_id_to_idx[cat['id']] = class_names_list.index(cat_name)
                else:
    ...
    if not cat_id_to_idx:
        ...
        for cid in all_class_ids:
            cid_str = str(cid)
            if cid_str in class_names_list:
                cat_id_to_idx[cid] = class_names_list.index(cid_str)
            else:
                cat_id_to_idx[cid] = cid - 1
